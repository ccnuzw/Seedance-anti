...
export function registerExportHandlers(): void {
  ...
  ipcMain.handle(IPC.EXPORT_PROMPTS, async (_event, params: {
    ...
    try {
      const { projectPath, projectName, episodeRange, format } = params
      ...
      return { success: true, path: result.filePath, count: episodeRange.length }
    } catch (error) {
  ...
  ipcMain.handle(IPC.EXPORT_ALL, async (_event, params: {
    ...
    try {
      ...
      // 复制 outputs
      const outputsDir = join(params.projectPath, 'outputs')
      await copyDirRecursive(outputsDir, join(exportDir, 'outputs'))

      // 复制 assets
      const assetsDir = join(params.projectPath, 'assets')
